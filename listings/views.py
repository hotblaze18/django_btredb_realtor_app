...
# 1) created simple views returning render with link to template
# 2) added a context(dict) to pass values to template after fetching
#    from database by Model.objects.all()
# 3) imported paginator and related modules from django.core to implement
#    pagination
# 4) used order by and then filter to order by date and display if published
